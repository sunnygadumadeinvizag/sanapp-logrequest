...
export async function GET(request: NextRequest) {
  const BASE_PATH = process.env.NEXT_PUBLIC_BASE_PATH || process.env.BASE_PATH || "/logrequest";
  ...
  const storedState = request.cookies.get("app5_oauth_state")?.value;
  ...
  try {
    ...
    if (!access.allowed) {
      ...
      res.cookies.delete("app5_oauth_state");
      return res;
    }

    // 3. Local role — App5 owns its own roles. Existing users keep their
    ...
    const returnTo = request.cookies.get("app5_return_to")?.value ?? "/";
    const safeReturn =
    ...
    res.cookies.delete("app5_return_to");
    res.cookies.set("app5_session", session, {
      httpOnly: true,
      sameSite: "lax",
      path: "/",
      maxAge: 60 * 60 * 8,
    ...
    console.error("App5 SSO callback failed:", err);
